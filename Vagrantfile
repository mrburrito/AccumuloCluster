...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  # Every Vagrant virtual environment requires a box to build off of.
    ...
    config.vm.provision "shell", path: "scripts/admin.sh"
    config.vm.provision "shell", path: "scripts/foldConf.sh", privileged: false
    config.vm.provision "shell", path: "scripts/hadoopConf.sh",  privileged: false
    config.vm.provision "shell", path: "scripts/zookeepConf.sh", privileged: false
    config.vm.provision "shell", path: "scripts/accumuloConf.sh", privileged: false

    config.vm.define "mnode" do |mnode|
        mnode.vm.network "private_network", ip: "192.168.15.2"
        mnode.vm.hostname = "mnode"
    end

  config.vm.define "snode1" do |snode1|
        snode1.vm.network "private_network", ip: "192.168.15.3"
        snode1.vm.hostname = "snode1"
  end

  config.vm.define "snode2" do |snode2|
        snode2.vm.network "private_network", ip: "192.168.15.4"
        snode2.vm.hostname = "snode2"
  end

  config.vm.define "snode3" do |snode3|
        snode3.vm.network "private_network", ip: "192.168.15.5"
